...
def generate_script_header(phone_model, file):  # Add git author a day
...
# ui_print("Region: GDPR");
# ui_print("Android 12 - RUI 3 C.08 GDPR");
...
def generate_flash_script(files, files_fw):
    ...
    generate_script_header("RMX3371", flash_script)
    for mbn in files:
        if (mbn.split(".")[1] != "elf"):
            flash_script.write(
                'package_extract_file("firmware-update/{file}", "/dev/block/bootdevice/by-name/{partition}");\n'.format(
